...
let humanFCount = humanOfficers.filter{femaleOfficers.contains($0)}.count

let percentHF = Float(humanFCount)/Float(humanCount)



// Q2: How many first officers are human?

let firstOfficerNames = firstOfficers.map{$0.key}

let firstOfficerHuman = firstOfficerNames.filter{humanOfficers.contains($0)}
...
let vulcanFirstOfficer: [Officer] = firstOfficers.filter{vulcanOfficers.contains($0.key)}.map{ Officer(name: $0, ship: $1)}
print(vulcanFirstOfficer.map{$0.name})


// Q7: How many these Vulcan first officers have names shorter than 5 characters?

let fiveChar = vulcanFirstOfficer.filter{ $0.name.count < 5 }
print(fiveChar.map{$0.name})
print(fiveChar.map{$0.name}.count)
